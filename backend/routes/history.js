...
historyRouter.post('/:id/reanalyse', async (req, res) => {
  try {
    ...
    if (fetchError || !row) return res.status(404).json({ error: 'Analysis not found.' });

    if (!bucketName) return res.status(500).json({ error: 'Storage not configured.' });

    const [frontRes, backRes] = await Promise.all([
      supabase.storage.from(bucketName).download(`${id}/front.jpg`),
      supabase.storage.from(bucketName).download(`${id}/back.jpg`),
    ]);
    if (frontRes.error || !frontRes.data) return res.status(400).json({ error: 'Front image not found in storage.' });
    if (backRes.error || !backRes.data) return res.status(400).json({ error: 'Back image not found in storage.' });
